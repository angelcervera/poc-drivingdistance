...
class ShardStorageTest extends TestKit(ActorSystem("ShardManagerActorTest"))
  with ImplicitSender
  with TestUtilities
  with WordSpecLike
  ...
  "A ShardStorage actor" should {
    ...
    "return Way by id using Get message" in {
      // With data
      val noEmptyShard = TestActorRef(ShardStorage.props(Map(1L->way1, 2L->way2)))
      noEmptyShard ! Get(1)
      noEmptyShard ! Get(2)
      noEmptyShard ! Get(3)
      expectMsg(Some(way1))
      ...
      // with no data
      val emptyShard = TestActorRef(ShardStorage.props)
      emptyShard ! Get(1)
      expectMsg(None)
    ...
    "add or replace ways using Put" in {
      val shard = TestActorRef(ShardStorage.props)
      shard ! Put(way1)
      shard ! Put(way2)
      shard ! Put(way3)
      shard ! Put(way2bis)
    ...
    "remove way from the shard using Delete message" in {
      val shard = TestActorRef(ShardStorage.props(Map(1L->way1, 2L->way2)))
      shard ! Delete(2L)
      shard ! Get(1)
      shard ! Get(2)
      shard ! GetMetrics
